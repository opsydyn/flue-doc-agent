import { Brand, Optic, Result } from "effect"
...
export const httpUrl = Optic.makePrism<string, HttpUrl>(
	(s) =>
		/^https?:\/\/.+/i.test(s)
			? Result.succeed(s as HttpUrl)
			: Result.fail(`not a valid HTTP URL: ${s}`),
	(u: HttpUrl): string => u,
)
...
export const httpStatusCode = Optic.makePrism<number, HttpStatusCode>(
	(n) =>
		Number.isInteger(n) && n >= 100 && n < 600
			? Result.succeed(n as HttpStatusCode)
			: Result.fail(`not a valid HTTP status code: ${n}`),
...
export const pageViews = Optic.makePrism<number, PageViews>(
	(n) =>
		Number.isInteger(n) && n >= 0
			? Result.succeed(n as PageViews)
			: Result.fail("page views must be a non-negative integer"),
...
export const pageviewThreshold = Optic.makePrism<number, PageviewThreshold>(
	(n) =>
		Number.isInteger(n) && n > 0
			? Result.succeed(n as PageviewThreshold)
			: Result.fail("pageview threshold must be a positive integer"),
